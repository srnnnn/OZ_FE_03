...
form.addEventListener('submit', function (event) {
  ...
  let userSsnF=event.target.idnumber1.value;
  let userSsnB=event.target.idnumber2.value;
  let userId=event.target.id.value;
  let userPwd1=event.target.pwd1.value;
  let userPwd2=event.target.pwd2.value;
  let userEmailF=event.target.emailF.value;
  let userEmailB=event.target.emailB.value;
  let userAddreess=event.target.address.value;
  let userGender=event.target.gender.value;
  let userAgree=event.target.agree.value;

    if(!(userId.length >=4 && userId.length <= 8)){
        alert("아이디는 4글자 이상 8글자 이하로 작성해주세요");
        return;
    }
